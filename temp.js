		...
		//			'created_day' : result[i]._doc.created_day
			...
			setTimeout(function(){
				...
				.on("data", function(data){					
					...
					dbm.ApplicationModel.find(/*{$and:[{'email':output[j].email}, {'geoLng':output[j].geoLng}, {'geoLat':output[j].geoLat}]},*/ function (err, result) {
						for(var j=0; j<output.length; j++){
							for(var i=0; i<result.length; i++){
								if((output[j].email == result[i]._doc.email) && (output[j].geoLng == result[i].geoLng) && (output[j].geoLat == result[i].geoLat)){
									var data2 = {
										'email' : result[i]._doc.email, 
										'teamname' : result[i]._doc.teamname,
										'region' : [result[i]._doc.region[0]]+[result[i]._doc.region[1]],
										'place' : result[i]._doc.place,
										'move' : result[i]._doc.move,
										'age' : result[i]._doc.age,	
										'gender' : result[i]._doc.gender,
